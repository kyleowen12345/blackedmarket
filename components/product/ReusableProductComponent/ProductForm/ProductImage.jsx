...
const ProductImage = ({productId,storeId,nextStep,product,prevStep}) => {
     ...
     onCompleted:data => {
       if(data){
        ...
        toast({
          ...
          description:product ? "Succesfully updated your product's image" : "Successfully added product image",
          status:"success",
          position:"top-right",
          isClosable: true,
        })
    ...
    useEffect(async() => {
        if(url){
            await productimage({variables:{id:productId,image:url},context:{headers:{token:authToken || ""}},refetchQueries:[{query:PRODUCTINFO,variables:{id:productId}},{query:STORESINFO,variables:{id:storeId}}]})
        } 
