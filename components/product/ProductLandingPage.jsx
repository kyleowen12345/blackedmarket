...
const ProductLandingPage = ({products}) => {
    ...
    return (
        <>
        <Grid  templateColumns={[ "repeat(2, auto)", "repeat(2, auto)", "repeat(3, auto)", "repeat(4, auto)", "repeat(5, auto)" ]} gap={2} >
            {products?.map(i=>(
                <NextLink key={i.id || i._id} href={`/products/info/${i.id}`}  passHref={true}>
                    <Box maxW="230px" h={["205px" ,"245px" ,"260px"]} borderWidth="1px" overflow="hidden" justifyContent="center" alignItems="center" as="a" bg="white" _hover={{border: "3px solid rgb(254,189,105)",}}>
                        <Image src={i.image} alt={i.productName} width={"100%"} height={"65%"} />
                          <Box pl={2}>
                                 <Box mt="1" fontWeight="semibold"as="h4" lineHeight="tight" isTruncated >
                                      <Text fontSize={["0.75rem" ,"0.875rem",]}>{i.productName}</Text>
                                 </Box>
