      ...
      <Box  display="flex" justifyContent="center" alignItems="space-between">
        <Flex width={1200} alignItems="center" justifyContent="center">
              ...
              <Menu >
                <MenuButton   bg="#000000">
                  ...
                  {WithUserLinks.map((link) => (
                   <MenuItem key={link.name}><Icon as={link.icon} color="#000000" /><Link  href={link.link} fontSize="md" color="#000000" p={2} >{link.name}</Link></MenuItem>
                     ))}
           ...
           <Container
              ...
              m={1}
           >
               <Link href="/user/cart/1"><Icon as={AiOutlineShoppingCart} color="#ffffff" w={7} h={7}/></Link> 
            </Container>
         </Flex>
        ...
        <DrawerContent bg="#ffffff">
          <DrawerCloseButton color="#000000"/>
           <DrawerHeader borderBottomWidth="1px" color="#000000">
            ...
            {WithUserLinks.map((link) => (
             <Container key={link.name} mb={5}> <Icon as={link.icon} color="#000000" /><NextLink key={link.name} href={link.link}><Link fontSize="md" color="#000000" p={2} onClick={onClose}>{link.name}</Link></NextLink> </Container>
              ))}
             </DrawerBody>
        </DrawerContent>
      </Drawer>
