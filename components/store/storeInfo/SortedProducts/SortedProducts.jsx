...
const SortedProducts = ({product}) => {
  ...
  return (
        <Box width={["100%","100%","100%","100%","100%",1200]} mr="auto" ml="auto"   >
            ...
            <Box bg="rgb(237,237,237)" mt={5} mb={5} p={3} pl={[1,1,1,3]} pr={[1,1,1,3]} display="flex" alignItems="center" width="100%">
                 ...
                 <Box display="flex"  alignItems="center" justifyContent="space-between" w={["90%","90%","90%","50%","50%"]}>
                    {sorterList.map(i=>(
                        <NextLink key={i.link} href={`/stores/info/products?store=${store}&id=1&sortOrder=${i.link}`} passHref>
                              <Link  fontSize={["10px","10px","13px","15px"]} color={sortOrder == i.link && "white"} bg={sortOrder == i.link ? "#FC8E00" : "white"} fontWeight="bold" p={2} w="100%" ml={2}>{i.name}</Link>
                        </NextLink>
                        ))}
                 </Box>
            </Box>
